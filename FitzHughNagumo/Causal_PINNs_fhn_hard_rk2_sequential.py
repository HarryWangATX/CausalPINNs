"""
Causal PINNs (hard RK2) for FitzHugh-Nagumo — sequential training.

Window k+1's IC comes from window k's prediction at t=T1. Single-GPU, no
multiprocessing.

Ansatz: u(t) = RK2(IC, t) + tau^CORRECTION_POWER * NN(tau), activation = SiLU,
with tau = t / T1, RK2_SUBSTEPS substeps.
"""
...
# ---------------------------------------------------------------------------
# FitzHugh-Nagumo system (scipy — pure numpy)
# ---------------------------------------------------------------------------
N_DIM = 2
A = 0.7
B = 0.8
...
def main():
    ...
    states_ref = scipy_odeint(fhn_rhs, STATE0, t_ref)
    ...
    for k in range(NUM_WINDOWS):
        ...
        out_dir = os.path.join(
            ...
            '..', 'hard_rk2_causalpinn_fhn_sequential',
        )
