...
for k in range(int(T / t1)):
    ...
    import os
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'original_causalpinn_lorenz')
    os.makedirs(out_dir, exist_ok=True)
    np.save(os.path.join(out_dir, 'x_pred_list.npy'), x_pred_list)
    np.save(os.path.join(out_dir, 'y_pred_list.npy'), y_pred_list)
    np.save(os.path.join(out_dir, 'z_pred_list.npy'), z_pred_list)
    np.save(os.path.join(out_dir, 'params_list.npy'), params_list)
    onp.save(os.path.join(out_dir, 'losses_list.npy'), onp.array(losses_list, dtype=object), allow_pickle=True)
    # Error
