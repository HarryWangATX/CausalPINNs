"""
Causal PINNs (baseline) for Lorenz-96 — sequential training.
...
Ansatz: state = NN(t) * t + IC, activation = tanh
"""
...
# ---------------------------------------------------------------------------
# Lorenz-96 system (scipy — pure numpy)
# ---------------------------------------------------------------------------
N_DIM = 5
FORCING = 8.0
...
def main():
    ...
    states_ref = scipy_odeint(lorenz96_rhs, STATE0, t_ref)
    ...
    out_dir = os.path.join(
        ...
        "..",
        "baseline_causalpinn_lorenz96_sequential",
    )
    ...
    for k in range(NUM_WINDOWS):
        ...
        err = onp.linalg.norm(state_preds_so_far[:n] - states_ref[:n]) / onp.linalg.norm(
            states_ref[:n]
        )
        ...
        for i, e in enumerate(per_dim, start=1):
            print(f"    x{i}: {e:.3e}")
